...
const CompanyMain = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://${host}:9090/offers/progress?check=${check}&uno=${myUno}`);
  ...
  const toggleExpand = async (index, ono) => {
    ...
    if (!expanded[index]) {
      try {
        const res = await axios.get(`http://${host}:9090/offers/selectByOno?ono=${ono}`);
  ...
  const deleteProgress = async () => {
    ...
    const url = `http://${host}:9090/offers/deleteByOno?${query}`;
