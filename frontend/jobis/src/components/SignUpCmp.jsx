...
const SignUpCmp = () => {
  ...
  const CheckId = async () => {
    ...
    try {
      const res = await axios.get(`http://${host}:9090/user/findUserId`, {
        params: { id: id },
      });
      if (res.data === 0) {
  ...
  const FindCmp = async () => {
    ...
    try {
      const res = await axios.get(`http://${host}:9090/user/checkComp`, {
        params: { crno: crno },
      });
      const data = res.data;
  ...
  const handleSignUp = async () => {
    ...
    try {
      const res = await axios.post(`http://${host}:9090/user/insertCUser`, {
        id: id,
        pw: pw,
        email: email,
        crno: crno,
