...
const KakaoCallback = () => {
  ...
  useEffect(() => {
    ...
    axios.post(`http://${host}:9090/user/kakao/check`, { code: codeFromUrl }, {withCredentials:true})
      .then((res) => {
  ...
  // 👉 생년월일 제한 (출생연도 2005년까지 허용)
  const today = new Date();
  const maxBirth = new Date(today.getFullYear() - 19, 11, 31); // 12월 31일

  if (step === 'loading') return <Container>카카오 로그인 처리 중입니다...</Container>;
